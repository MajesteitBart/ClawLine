...
const CLIENT_ID = process.env.HAPPY_GEMINI_GOOGLE_CLIENT_ID ?? '';
const CLIENT_SECRET = process.env.HAPPY_GEMINI_GOOGLE_CLIENT_SECRET ?? '';
...
export async function authenticateGemini(): Promise<GeminiAuthTokens> {
    ...
    if (!CLIENT_ID || !CLIENT_SECRET) {
        throw new Error(
            'Missing Gemini OAuth credentials. Set HAPPY_GEMINI_GOOGLE_CLIENT_ID and HAPPY_GEMINI_GOOGLE_CLIENT_SECRET.'
        );
    }
